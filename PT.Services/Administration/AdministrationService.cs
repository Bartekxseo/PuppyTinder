...
namespace PT.Services.Administration
{
    public class AdministrationService : IAdministrationService
    {
        ...
        {
            var user = ptDbContext.Set<Domain.Entities.User>().Where(x => x.Username == username || x.Email == username).FirstOrDefault();
            if (user != null)
            {
        ...
        public UserViewModel getUser(string token)
        {
            ...
            var user = ptDbContext.Set<Domain.Entities.User>().Where(x => x.Token == token).FirstOrDefault();
        ...
        {
            var dbUser = user.Id.HasValue && user.Id.Value != 0 ? ptDbContext.Set<Domain.Entities.User>().SingleOrDefault(x => x.Id == user.Id) : new Domain.Entities.User();
            mapper.Map(user, dbUser);
            if(!(user.Id.HasValue && user.Id.Value!=0))
            {
                dbUser.Token = tokenGeneratorService.generateToken(20);
            }
            ptDbContext.Set<Domain.Entities.User>().AddOrUpdate(x=>x.Id,dbUser);
